`timescale 1us/1ns
// Final SDF stage: plain radix-2 butterfly (A +/- B), no rotation.
module last_sdf_stage #(
    parameter logic FLOAT_POINT = 1,
    parameter int   DATA_NB_BITS = 64,
    parameter int   DELAY_DEPTH  = 1
)(
    ...
    last_stage #(.FLOAT_POINT(FLOAT_POINT), .DATA_NB_BITS(DATA_NB_BITS)) u_bf (
        .clk,.rst_n,.en(en),
