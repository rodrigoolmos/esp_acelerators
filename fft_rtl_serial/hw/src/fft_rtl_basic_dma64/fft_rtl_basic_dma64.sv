...
//   * READ FSM  : issues bounded, one-frame DMA read requests. It waits for
//                 every frame's data channel to complete before requesting the
//                 next one, and keeps at most three frames ahead of completed
//                 writes so the ESP DMA socket can alternate reads and writes
//                 without deadlocking.
...
//   * COMPUTE request (load=0): one read transaction of FFT_SIZE beats per
//     frame brings the samples, which are windowed (using the coefficients
...
module fft_rtl_basic_dma64 #(
    ...
    //   * COMPUTE request (conf_info_window[1]=0): read one FFT_SIZE frame per
    //     DMA transaction; the normal compute path handles the rest.
    // ---------------------------------------------------
    typedef enum logic [2:0] {
        IDLE_R, REQ_R, WAIT_R, READ_DATA_R, WAIT_SLOT_R, LOAD_WAIT_R
    } state_read_e;
    state_read_e read_st;
    logic [$clog2(FFT_SIZE):0] read_counter;

    /*
     * The first result needs slightly more than one extra frame of active
     * tokens to leave the SDF pipeline. Three admitted input frames are enough
     * to produce a complete output frame. Holding the fourth request until an
     * output DMA completes prevents the read side from occupying the ESP
     * socket while both output buffers are full.
     */
    localparam logic [31:0] READ_AHEAD_FRAMES = 32'd3;
        ...
        if (!rst) begin
            ...
            case (read_st)
                ...
                REQ_R: begin
                    ...
                    // LOAD and COMPUTE requests are each exactly one frame.
                    dma_read_ctrl_data_length <= FFT_SIZE;
                    dma_read_ctrl_data_size   <= 3'b011; // 64-bit beats
                    dma_read_ctrl_data_user   <= 0;
                    dma_read_ctrl_data_index  <= win_load_r
                                               ? read_index_r
                                               : read_index_r + FFTs_read * FFT_SIZE;
                    read_st                   <= WAIT_R;
                end
                WAIT_R: begin
                    if (dma_read_ctrl_valid && dma_read_ctrl_ready) begin
                        ...
                        if (win_load_r) begin
                            ...
                            // A grant only acknowledges the control request.
                            // ESP has no queue for further requests here: wait
                            // for this frame's complete data channel before
                            // presenting the next control request.
                            read_counter <= 0;
                            read_st      <= READ_DATA_R;
                        end
                    end
                ...
                READ_DATA_R: begin
                    if (dma_read_chnl_valid && dma_read_chnl_ready) begin
                        if (read_counter == FFT_SIZE - 1) begin
                            read_counter <= 0;
                            FFTs_read    <= FFTs_read + 1'b1;

                            if (FFTs_read + 1'b1 >= conf_info_burst_len) begin
                                read_st <= IDLE_R;
                            end else if (FFTs_read + 1'b1 <
                                         FFTs_written + READ_AHEAD_FRAMES) begin
                                read_st <= REQ_R;
                            end else begin
                                read_st <= WAIT_SLOT_R;
                            end
                ...
                WAIT_SLOT_R: begin
                    // Leave the read request deasserted so a pending output
                    // write wins arbitration in the ESP DMA socket.
                    if (FFTs_read < FFTs_written + READ_AHEAD_FRAMES)
                        read_st <= REQ_R;
                end
                LOAD_WAIT_R: begin
